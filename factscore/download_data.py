...
def download_file(_id, dest, cache_dir):
    ...
    if os.path.exists(dest.replace(".zip", "")) or os.path.exists(
        os.path.join(cache_dir, dest.replace(".zip", ""))
    ):
        print("[Already exists] Skipping", dest)
        print(
            "If you want to download the file in another location, please specify a different path"
...
def recover_instruct_llama(
    path_raw, output_path, device="cuda", test_recovered_model=False
):
    ...
    tokenizer_recovered = transformers.AutoTokenizer.from_pretrained(
        "kalpeshk2011/instruct-llama-7b-wdiff"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--llama_7B_HF_path", type=str, default=None)
    ...
    download_file(
        "1mekls6OGOKLmt7gYtHs0WGf5oTamTNat", "enwiki-20230401.db", args.data_dir
    )

    if args.llama_7B_HF_path:
        recover_instruct_llama(
            args.llama_7B_HF_path, os.path.join(args.model_dir, "inst-llama-7B")
        )

    # download the roberta_stopwords.txt file
    subprocess.run(
        [
            "wget https://raw.githubusercontent.com/shmsw25/FActScore/main/roberta_stopwords.txt"
        ],
        shell=True,
    )
